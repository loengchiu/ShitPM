...
- 已按当前模板主骨架输出；按需章节仅在形成真实约束时出现，未为凑完整度机械补写
- 数据字典已按表格收敛核心字段、枚举与状态；业务来源口径符合 `prd-generation-rules.md`
- 页面说明按页面组织、按用户动作展开；每个动作已写清触发条件、操作过程、操作结果和异常处理
- 页面开头如需交代区域，只保留帮助理解页面空间感的区域职责概述，未退回按区域逐项盘点
- 正文保持规格说明口径，信息密度足够，无大白话、解释腔或设计说明腔
- 已根据内容性质混用自然段、`1. 2. 3.`、`·`，结构自然，不僵成单一句式
- 页面内部未退化成 UI 区域盘点；但页面级内容仍可回溯到 `Page Structure`
- 所有数值已写死具体数字，UI 文案直接写在描述中，无“按配置 / N秒 / 详见原型”之类占位
